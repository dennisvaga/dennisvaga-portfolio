...
export function Project({ project }: ProjectProps) {
  ...
  return (
    <div className="relative h-full w-full">
      {/* View Details Button - Outside card, top right */}
      <Button
        variant="outline"
        size="sm"
        asChild
        className={cn(
          `absolute -top-12 z-20 shadow-lg gap-2`,
          isRtl ? "-left-0" : "-right-0"
        )}
      >
        <Link
          href={`/projects/${project.id === "bodyfuel-admin" ? "bodyfuel" : project.id}`}
        >
          <ArrowUpRight className={cn(`h-4 w-4`, isRtl ? "rotate-270" : "")} />
          {t("viewDetails")}
        </Link>
      </Button>
